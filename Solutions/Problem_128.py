'''
Question -->
The Tower of Hanoi is a puzzle game with three
rods and n disks, each a different size.

All the disks start off on the first rod in a stack.
They are ordered by size, with the largest disk on
the bottom and the smallest one at the top.
...
the rods are numbered, with the first rod being 1,
...
Approach -->
Recursively Move n-1 disks to aux rod and nth rod to final rod
'''


def tower_of_hanoi(n, from_rod=1, aux_rod=2, to_rod=3):

    if n == 1:
        print("Move", from_rod, "to", to_rod)
        return
